...
# 1. LOAD EXCEL FILE
# ============================================================
path = r"C:\Users\aliah\Box\RL Project\BD_Election_Final.xlsx"
...
# ============================================================
# 6. POLL PRIOR (OLD POLL + NEW POLL BLENDED)
# ============================================================

# -----------------------------
# InnoVision POLL (Decided + Undecided)
# -----------------------------
decided_BNP = 0.413
decided_JeI = 0.303
decided_AL  = 0.188
...
# Undecided split rule (user-configurable)
und_BNP = 0.50
und_JeI = 0.50

old_poll = {
    "Bangladesh Nationalist Party":
        f_decided * decided_BNP + und_BNP * f_undecided,

    "Jamat-E-Islami Bangladesh":
        f_decided * decided_JeI + und_JeI * f_undecided,
...
# Normalize
S_old = sum(old_poll.values())
old_poll = {k: v / S_old for k, v in old_poll.items()}



# --------------------------------
# NEW POLL (Direct Preference) IRI
# --------------------------------
...
# STEP 1: Process committed votes
committed_BNP = new_raw["BNP"]
committed_JeI = new_raw["JeI"]
committed_JP  = new_raw["JatiyaParty"]

# All small parties grouped into "Other"
committed_other = new_raw["NCP"] + new_raw["IAB"] + new_raw["OtherSmall"]

# STEP 2: Undecided voters (DN + Refused)
undecided = new_raw["DontKnow"] + new_raw["Refused"]

# Apply undecided split (uses the SAME rule for consistency)
BNP_new = committed_BNP + und_BNP * undecided
JeI_new = committed_JeI + und_JeI * undecided
JP_new  = committed_JP
Other_new = committed_other

# STEP 3: Remove explicit non-voters (NoVote) from normalization
S_new = BNP_new + JeI_new + JP_new + Other_new

new_poll = {
    "Bangladesh Nationalist Party": BNP_new / S_new,
    "Jamat-E-Islami Bangladesh":   JeI_new / S_new,
    "Jatiya Party":                JP_new / S_new,
    "Other":                       Other_new / S_new,
}
...
poll_prior = {
    p: w_old * old_poll[p] + w_new * new_poll[p]
    for p in ["Bangladesh Nationalist Party",
              "Jamat-E-Islami Bangladesh",
              "Jatiya Party",
              "Other"]
}

print("\n=== OLD Poll Prior (decided + undecided model) ===")
print(old_poll)

print("\n=== NEW Poll Prior (direct preference + undecided only) ===")
print(new_poll)

print("\n=== Final Blended Poll Prior (40% old poll, 60% new poll) ===")
print(poll_prior, "\n")
